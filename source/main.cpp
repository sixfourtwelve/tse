...
public:
    ...
    void CookAssets(void)
    {
        ...
        const std::set<std::string> sourceExts = { ".gltf", ".glb", ".obj", ".fbx", ".dae" };
        for (const auto& entry : Fs::directory_iterator("assets/models"))
        {
            ...
            if (Fs::exists(meshOut) && Fs::last_write_time(meshOut) >= Fs::last_write_time(src))
                continue;

            Ogre::LogManager::getSingleton().logMessage("COOK: " + src.string());

            auto mesh = Ogre::MeshManager::getSingleton().load(src.filename().string(), "Game");
            try
            {
            ...
            if (!Fs::exists(materialOut))
            {
                ...
                for (auto* sub : mesh->getSubMeshes())
                {
                    if (!queued.insert(sub->getMaterialName()).second)
                        continue;
                    auto mat = Ogre::MaterialManager::getSingleton().getByName(sub->getMaterialName());
                    if (mat)
